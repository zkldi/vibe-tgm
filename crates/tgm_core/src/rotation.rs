...
use crate::piece::{PieceKind, RotIndex, piece_cells, rotate_ccw, rotate_cw};
...
fn try_rotate(
	...
	if kind == PieceKind::I {
		...
		// Vertical I spans four rows; at the horizontal spawn height the column would clip the
		// ceiling — same case as [`crate::piece::spawn_origin`], shift origin down one row.
		if !collides_def(board, px, py - 1, &new_def) {
